...
export default function FAQsPage() {
  ...
  const [activeCategory, setActiveCategory] = useState(data.fans[0] ? data.fans[0].id : null);
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        setData(parsed);
        setActiveCategory(parsed.fans[0] ? parsed.fans[0].id : null);
  ...
  const handleExportData = () => {
    const fullCode = BOILERPLATE + JSON.stringify(BOILERPLATE) + ";\\n\\nconst REST_OF_CODE = " + JSON.stringify(REST_OF_CODE) + ";\\n\\nconst defaultFaqsData = " + JSON.stringify(data, null, 2) + ";" + REST_OF_CODE;
    navigator.clipboard.writeText(fullCode);
    alert("Entire page.jsx code copied to clipboard! Open src/app/faqs/page.jsx, select all, paste, and save.");
  };
    ...
    return (
      <div className={"border border-white/10 bg-[#111] rounded-2xl overflow-hidden transition-all duration-300 hover:border-gray-700 mb-4 group relative"}>
        ...
        <div ref={contentRef} className="transition-all duration-300 ease-in-out overflow-hidden" style={{ maxHeight: isOpen ? (contentRef.current?.scrollHeight + "px") : "0px" }}>
          <div className="px-6 pb-6 text-gray-400 font-medium leading-relaxed border-t border-white/5 mt-2 pt-4 whitespace-pre-wrap">
            {faq.a}
          </div>
        </div>
    ...
    <div className="animate-in fade-in slide-in-from-bottom-8 duration-700 bg-[#0a0a0a] min-h-screen flex flex-col font-sans selection:bg-[#a3e635] selection:text-black">
      ...
      {isAdmin && (
        <div className="fixed top-[70px] left-0 w-full bg-[#a3e635] text-black text-xs font-black uppercase tracking-widest text-center py-2 z-50 flex items-center justify-center gap-4 shadow-lg shadow-[#a3e635]/20">
          ...
          <button onClick={handleExportData} className="bg-black text-white px-3 py-1 rounded hover:bg-gray-900 flex items-center gap-1.5"><Copy size={12}/> Export Entire Page Code</button>
